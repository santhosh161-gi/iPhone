...
export default function ProductsCarousel() {
  ...
  return (
    <div className="relative w-full mt-10 px-2 sm:px-6">
      ...
      <div
        ref={sliderRef}
        className="flex gap-6 sm:gap-8 overflow-x-auto sm:overflow-x-hidden scroll-smooth no-scrollbar py-4
                   snap-x snap-mandatory"
      >
        {products.map((p) => (
          <div
            key={p.id}
            className="flex-shrink-0 w-[80%] sm:w-[45%] md:w-[32%] lg:w-[26%] xl:w-[30%]
                       bg-transparent rounded-3xl flex flex-col items-center text-center
                       snap-start"
          >
            {/* IMAGE */}
            <img
              src={p.image}
              alt={p.name}
              className="h-[200px] sm:h-[250px] md:h-[280px] lg:h-[320px] xl:h-[350px]
